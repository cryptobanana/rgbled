...
1   white drinking straw  -- not used for Dan
1   IR333-A IR emitter    -- not used for Dan
1   TSOP32138 IR detector -- not used for Dan
2   47 ohm resistors
1   1k ohm resistor
1   0.1 uF capacitor
...
void sendrgbElement( int index ) {
  ...
  for (int fadeCounter=1; fadeCounter<=FadeTime; fadeCounter++) {
    ...
    pulseIR();             // For Dan: this function now does nothing -- pulse the IR emitter for a little bit (if there's a reflection to the IR detector as a result, then the ISR will set Start_Over = 1)
...
int main(void) {
  ...
  DDRB = 0b00010110;    // setting a bit to 1 makes it an output, setting a bit to 0 makes it an input
                        //   PB5 (unused) is input
